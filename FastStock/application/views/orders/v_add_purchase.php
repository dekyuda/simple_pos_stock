...
<?php
	$datacode = $purchase['id_stock_in'];	

	if($datacode){
		$code_val = substr($datacode, 4);
		$code = (int)$code_val;
		$code = $code + 1;
		$auto_code = "PO-".str_pad($code, 4, "0", STR_PAD_LEFT);
	}
	else{
		$auto_code = "PO-0001";
	}
...
<script>
	$(document).ready(function(){
		...
		function purchase_grandtotal(){
			...
			grand = parseInt(sub)+parseInt(delv)+parseInt(tax)-parseInt(disc);
		...
		$(document).on('change keyup keypress', '.product_name', function(){
			...
			$.ajax({
				dataType: 'json',
				url: '<?php echo base_url()?>purchase/product',
				type: 'post',
				data: {id_product: val},
				success: function(response){
					$('.cost.counter'+counter).val(response.cost);
